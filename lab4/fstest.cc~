...
int SetCopyTime(char *from, OpenFile *openFile, bool bWriteBack = false)
{
    struct stat st;
...
//拷贝完UNIX文件数据之后立刻回写Nachos的文件头
    ASSERT(!SetCopyTime(from,openFile,true));
...
//	ASSERT(start == openFile->Length());
    ...
    //如果新文件不存在，那么相当于是创造了一个原文件的副本，那么新文件的修改时间是和原文件一致的
    if(!ToExists)
...
NAppend(char *from, char *to)
{
    ...
    openFileTo->Seek(start);		//更新光标位置
    
// Append the data in TransferSize chunks
    buffer = new char[TransferSize];
    openFileFrom->Seek(0);		//从0开始复制
