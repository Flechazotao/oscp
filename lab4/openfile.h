...
class OpenFile {
  public:
    ...
    void WriteBack(void);   		// 关闭文件时，写回文件头数据
    ...
    time_t GetModTime();  		// 这里面直接调用文件头hdr的GetModTime()，相当于起到一个封装的作用

    void SetModTime(time_t modTime);  	// 这里面直接调用文件头hdr的SetModTime，相当于起到一个封装的作用
