...
//检查并处理所有到期中断			// interrupts disabled)
...
void 
Interrupt::Exec(int fileAddress){
    printf("正在执行Exec系统调用！\n");
    char fileName[50];
    int i = 0;

    do {
       machine->ReadMem(fileAddress + i, 1, (int*) &fileName[i]); 
    } while(fileName[i++] != '\0');
    ...
    machine->WriteRegister(2, (int)space->getSpaceID());
}
...
//		we're done!
//		advanceClock=true：系统处于空闲模式（IdleMode），没有就绪线程
//		advanceClock=false：系统正常执行，有线程在运行
//----------------------------------------------------------------------
bool
Interrupt::CheckIfDue(bool advanceClock)
{
	...
	DumpState();
//取出最早的中断
//SortedRemove 从排序列表中移除最早的元素，并返回其时间到 when
...
// 空闲模式：直接跳到中断时间
    ...
    } else if (when > stats->totalTicks) {	// not time yet, put it back  非空闲：放回列表，等待
	pending->SortedInsert(toOccur, when);
	return FALSE;
    }

// Check if there is nothing more to do, and if so, quit
// 系统空闲，只有定时器中断（TimerInt）等待，没有其他中断，就绪队列为空，无线程可切换
...
//执行中断处理函数
//相当于调用“某个设备”的“中断处理函数”		// running in the kernel
    (*(toOccur->handler))(toOccur->arg);
// 恢复上下文					// call the interrupt handler
